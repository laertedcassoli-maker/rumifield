...
interface InstallationObservationsBlockProps {
  stageId: string;
...
export default function InstallationObservationsBlock({ 
  stageId, 
  ...
  hasReport = false,
  onChange,
}: InstallationObservationsBlockProps) {
  ...
  const updateNotesMutation = useMutation({
    mutationFn: async ({ internalNotes, publicNotes }: { internalNotes: string; publicNotes: string }) => {
      const { data, error } = await (supabase as any)
        ...
        .update({
          observacao_interna: internalNotes || null,
          observacao_externa: publicNotes || null
        })
        .eq('id', stageId)
        .select('id');

      if (error) throw error;
      if (!data?.length) throw new Error('Sem permissão para salvar as observações.');
    },
    onSuccess: () => {
      setSaveStatus('saved');
  ...
  const debouncedSave = useDebouncedCallback((internal: string[], pub: string[]) => {
    ...
    onChange?.([...internal, ...pub].some(l => l.trim()));
        ...
        <CollapsibleContent className="animate-accordion-down data-[state=closed]:animate-accordion-up">
          <CardContent className="pt-0 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                {hasReport ? 'Contexto adicional sobre a visita. Será exibida no relatório enviado ao produtor.' : 'Contexto adicional sobre a visita.'}
              </p>

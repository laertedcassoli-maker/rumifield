...
export default function InstallationVisitMediaUpload({ 
  stageId, 
  isCompleted = false,
  onStatusChange 
}: Props) {
  ...
  const { data: mediaItems = [], isLoading } = useQuery({
    queryKey: ['installation-media', stageId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('installation_visit_media' as any)
        .select('*')
        .eq('stage_id', stageId)
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const filePath = `${user.id}/installation/${stageId}/${fileName}`;
      ...
      // Refresh the list
      await queryClient.invalidateQueries({ queryKey: ['installation-media', stageId] });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['installation-media', stageId] });

...
- [ ] Instalações — fase de execução (UI de instalações + execução de checklist via motor existente); schema já aplicado (installations/installation_stages/installation_checklists). Planejamento solicitado pelo usuário. Ajustes confirmados: tecnico_campo no menu (só execução da própria etapa, filtro na UI) e execução offline-first (Dexie + syncQueue, padrão useOfflineChecklist).
